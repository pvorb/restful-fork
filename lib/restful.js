...
var director = require('director'),
    ...
    utile    = require('utile'),
    http     = require('http'),
    defaultOperator = require('./defaultOperator.js');

exports.operator = defaultOperator;
...
exports.buildResourceId = function(resources, ids) {
  ...
  //
  // Director is for some reason adding a function to the end of the handler
  // arguments.
...
exports.extendRouter = function (router, resources, options, respond) {
  ...
  var operator = options.operator || defaultOperator;
  ...
  router.path(options.prefix, function() {
    ...
    if (options.explore) {
      //
      // Bind GET / to a generic explorer view of routing map ( using
      // `director-explorer` )
      //
      this.get('/', function () {
        var rsp = '';
        //
        // Remark: Output the basic routing map for every resource using
        // https://github.com/flatiron/director-reflector
        //
        rsp += de.table(self);
        this.res.end(rsp);
      });
    ...
    _extend(this, resources, options, operator, respond);
  })
};

function _extend (router, resources, options, operator, respond, routeInfo) {
    ...
    router.path(entity, function () {
      ...
      this.get(function () {
        ...
        if (options.parent) {
          ...
          operator.apply('show all', resource.name, resource, req, showHandler);
      ...
      this.post(function () {
        ...
        operator.apply('create', resource.name, resource, req, createHandler);
      ...
      this.path('/' + param, function () {
        ...
        if (resource._children && resource._children.length > 0) {
          ...
          resource._children.forEach(function(child){
            ...
            _extend(paramScopedRouter, childResource, clonedOptions, operator, respond, childRouteInfo);
          });
        }
        ...
        this.post(function () {
          ...
          operator.apply('create', resource.name, resource, req, respond);
        });
        ...
        this.get(function () {
          ...
          req.options = utile.clone(options);

          if (!options.strict)
            preprocessRequest(req, resource, 'show');

          req.id = exports.buildResourceId(routeInfo, arguments);
           
          var handler = function (err, result) {
            if (err)
              return respond(req, res, 404, resource.lowerResource, req.id);
            
            respond(req, res, 200, result);
          };

          operator.apply('show', resource.name, resource, req, handler);
        });
...
//
// ### @private function _extendWithNonStrictRoutes (router, resource, options,
// respond)
...
// #### @respond function
//
// Since not all HTTP clients support PUT and DELETE verbs ( such as forms in
// web browsers ),
...
//
// see:
// https://github.com/senchalabs/connect/blob/master/lib/middleware/methodOverride.js
//
...
// #### @value {Object} **Optional** Value to set in the result for the
// specified `key`
//
